...
// This uses koge stone approach with double shared memory buffering to do the scan 
__global__ void scanKernel(float *input, float *vector, float * partial_sum){
   int i = blockIdx.x * blockDim.x + threadIdx.x;

     __shared__ float share1[NUM_BLOCK];
     __shared__ float share2[NUM_BLOCK];
     ...
     for(int stride=1; stride <= THREAD_PER_BLOCK/2; stride *=2){
          if(threadIdx.x >= stride){
            b2[threadIdx.x]=b1[threadIdx.x]+b1[threadIdx.x-stride];            
          }else{
            b2[threadIdx.x]=b1[threadIdx.x];            
     ...
     partial_sum[blockIdx.x]=b1[threadIdx.x];
     }
     vector[i]=b1[threadIdx.x];
...
// free memories allocated on the hsot
delete[] vector_h;
